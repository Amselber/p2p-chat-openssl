...
#define FP1 "a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b"
#define FP2 "b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c"
#define FP3 "c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d"

// Имена
#define FP1_NAME "alice"
#define FP2_NAME "bob"
#define FP3_NAME "marty"

void setUp_node(void) { node_reset(); }

// Тест: добавление новой ноды
void test_node_add_new(void) {
  node_t *n = node_add(FP1, FP1_NAME, 5, 0);
...
void test_node_add_update(void) {
  node_t *n1 = node_add(FP1, FP1_NAME, 5, 1);
  TEST_ASSERT_NOT_NULL(n1);
  TEST_ASSERT_EQUAL_STRING(FP1_NAME, n1->name);
  TEST_ASSERT_EQUAL(5, n1->fd);

  // Добавляем ту же ноду с новым fd
  node_t *n2 = node_add(FP1, FP1_NAME, 7, 0);
...
void test_node_add_two_different(void) {
  node_t *a = node_add(FP1, FP1_NAME, 5, 1);
  node_t *b = node_add(FP2, FP2_NAME, 6, 0);
  ...
  TEST_ASSERT_EQUAL_STRING(FP1_NAME, a->name);
  TEST_ASSERT_EQUAL_STRING(FP2_NAME, b->name);
...
void test_node_find_by_fd_found(void) {
  node_add(FP1, FP1_NAME, 10, 0);
  node_add(FP2, FP2_NAME, 20, 1);

  node_t *n = node_find_by_fd(20);
  TEST_ASSERT_NOT_NULL(n);
  TEST_ASSERT_EQUAL_STRING(FP2, n->fp);
  TEST_ASSERT_EQUAL_STRING(FP2_NAME, n->name);
}
...
void test_node_find_by_fd_minus_one(void) {
  node_add(FP1, FP1_NAME, -1, 1);

  node_t *n = node_find_by_fd(-1);
  TEST_ASSERT_NOT_NULL(n);
  TEST_ASSERT_EQUAL_STRING(FP1, n->fp);
  TEST_ASSERT_EQUAL_STRING(FP1_NAME, n->name);
}
...
void test_node_each_iterates_all(void) {
  node_add(FP1, FP1_NAME, 1, 1);
  node_add(FP2, FP2_NAME, 2, 1);
  node_add(FP3, FP3_NAME, 3, 1);
...
void test_node_fd_minus_one(void) {
  node_t *n = node_add(FP1, FP1_NAME, -1, 0);
  TEST_ASSERT_NOT_NULL(n);
